...
async def ddl_call_back(client, query):
    ...
    try:
        if "youtu" in youtube_dl_url or "youtube" in youtube_dl_url:
            ...
            xLAZY_BAAPUx_t_length = int(x_BAAPUx_d_size.headers.get("Content-Length", 0))
        ...
        if file_size > TG_MAX_FILE_SIZE:
            ...
            if (await db.get_upload_as_doc(query.from_user.id)) is False:
                ...
                await client.send_document(
                    ...
                    progress_args=(
                        script.UPLOAD_START.format(custom_file_name),
                        query.message,
                        start_time
                    )
                )
...
async def download_coroutine(bot, session, custom_file_name, url, file_name, chat_id, message_id, start):
    ...
    async with session.get(url, timeout=PROCESS_MAX_TIMEOUT) as response:
        ...
        x_BAAPUx_path = urlparse(url).path
        x_BAAPUx_u_name = os.path.basename(xLAZY_BAAPUx_path)
        ...
        await bot.edit_message_text(
            ...
            """.format(x_BAAPUx_u_name, humanbytes(total_length))
        )
        with open(file_name, "wb") as f_handle:
            while True:
                ...
                if round(diff % 5.00) == 0 or downloaded == total_length:
                    ...
                    xx_BAPUXX_total_size = humanbytes(total_length)
                    tp = round(percentage, 2)
                    xx_BAPUXX_estimated_total_time = TimeFormatter(milliseconds=estimated_total_time)
                    template_name = custom_file_name if custom_file_name else "**⚠ You haven't given any custom name...**"

                    xLDx = f"**ღ♡ ʀᴜɴɴɪɴɢ  ᴄᴏɴꜱᴛʀᴜᴄᴛɪᴏɴ ♡♪**\n**ᵉⁿʲᵒʸ ˢᵘᵖᵉʳᶠᵃˢᵗ ᵈᵒʷⁿˡᵒᵈ ᵇʸ [ᴰᵉᵛᵉˡᵒᵖᵉʳʳ](https://t.me/termuxro)◔_◔** \n\n**░░✩ 📂𝐎𝐑𝐆 𝐅𝐈𝐋𝐄𝐍𝐀𝐌𝐄 ✩ **\n<code>{xLAZY_BAAPUx_u_name}</code>\n\n**░░✩ 📝𝐍𝐄𝐖 𝐍𝐀𝐌𝐄 ✩ **\n<code>{template_name}</code>\n\n ☼﹍︿﹍ⲯ﹍︿﹍﹍︿﹍ⲯ﹍︿﹍☼\n⚡️**Done:{tp}**%| 🧬ѕιzє: {xx_BAPUXX_total_size}"
                    progress = "{0}{1}".format(
                    ...
                    tmp = xLDx + "\n" + progress + script.PROGRESS_BAR.format( 
                        ...
                        humanbytes(speed),
                        xxLAZY_BAPUXX_estimated_total_time if xx_BAPUXX_estimated_total_time != '' else "0 s"
                    )
                    try:
                        current_message = tmp
